...
9. **2026 工具链读法**：量化不是“选 GPTQ/AWQ 就结束”，而是 **格式 × 硬件 × kernel × calibration × serving runtime** 的组合。Hopper/Blackwell 上 FP8/NVFP4 要看 Transformer Engine、TensorRT-LLM、NVIDIA Model Optimizer 和实际 fused kernel 支持；同一 W4/FP4 名字在不同后端可能数值格式、scale 粒度和吞吐完全不同。
...
| Blackwell FP4/NVFP4 生产部署 | NVFP4 + Model Optimizer calibration/export | NVIDIA Model Optimizer + TensorRT-LLM |
...
**Quantization Quick Reference** · 主要参考：Dettmers 2022 (LLM.int8()), Frantar 2023 (GPTQ), Xiao 2023 (SmoothQuant), Lin 2024 (AWQ), Ashkboos 2024 (QuaRot), Lin 2025 (QServe), NVIDIA Transformer Engine / TensorRT-LLM / Model Optimizer docs, NVIDIA NVFP4 Blackwell notes. 最后更新：2026-06。
