...
6. **Web / GUI agent RL**：**WebRL** (Qi et al. 2024 ICLR-25 arXiv 2411.02337)——self-evolving curriculum + ORM + retrospective rollout，把 8B Llama 推到 WebArena 43%；**AgentQ** (Putta et al. 2024 arXiv 2408.07199)——MCTS 搜索 + AI critique + DPO offline 训练；**Computer-Use / browser / IDE agent**（Claude 3.5→Fable/Opus、GPT-5.5、Gemini 3.1、DeepSeek-V3.2 这一代）已经把截图、DOM、文件系统、shell、search/tool call 纳入产品能力。但闭源模型的能力提升 **不能反推出具体训练 recipe**：官方通常只公开能力和安全系统，不公开 PPO/GRPO/RM/verifier 细节。
...
|---|---|---|---|---|
| **TAU-bench (retail)** | customer service multi-turn | ~50% (GPT-4) | Claude Fable/Opus、GPT-5.5、Gemini 3.1 一代显著更强；具体数字要看私有 holdout | Agent SFT + tool reward + policy/search scaffold |
| **SWE-bench Verified / Multilingual / private holdout** | GitHub PR fix | ~25% (Claude 3.5) | frontier coding agents 已明显超过 2024；公开榜需警惕 contamination | Agent scaffold + test verifier + retrieval/planning |
| **OSWorld** | OS GUI task | ~12% (GPT-4V) | Computer-use 模型持续提升，但系统权限、安全过滤和任务定义强影响分数 | GUI grounding + sandbox + safety filter |
| **WebArena** | web nav | 14.4% (GPT-4) | 43% (WebRL Llama-8B) | curriculum + RL |
| **GAIA / BrowseComp 类综合助理** | general assistant | 15% (GPT-4) | 闭源 agent 能力持续刷新；公开数字常混入 scaffold 差异 | Tool use + search + verifier + long context |

注意：benchmark contamination 风险大；SWE-bench Verified 这类公开集适合做回归测试，不适合单独宣称“通用代码 agent SOTA”。当前更可信的比较要看 SWE-Lancer、SWE-bench Multilingual、company private holdout，以及是否统一 scaffold、上下文窗口、工具权限和重试预算。
...
- 2025-2026: Claude 3.7 / 4.x / Fable 5 / Opus 4.8，OpenAI GPT-5.5，Gemini 3.1 Pro Preview，DeepSeek-V3.2 都把 tool use、coding、长程 workflow 或 thinking in tool-use 放到核心能力里
- Action space 从文本 tool call 扩展到屏幕截图、鼠标键盘、浏览器/DOM、文件系统、shell、搜索、代码执行和 API 调用
- 公开 system card / docs 通常会说明安全过滤、工具权限、模型能力边界，但**不披露** 训练数据规模、RL 算法、reward 权重或 verifier 细节

**算法层面的 inferred 信息**（学术界推测，未官方确认）:

- 可能有人工演示、合成 rollout、偏好学习、rule/verifier reward、LLM-as-judge、安全 classifier、curriculum 等混合信号
- 可能用 PPO/GRPO/RLOO 或其它内部 RL；**没有官方确认时不能写成事实**
- 产品能力提升也可能来自更好的 base model、上下文窗口、工具协议、检索/规划 scaffold、sandbox 和 eval 数据，而不只是 RL 算法本身
...
> 💡 **2026Q1-Q2 趋势** — agent RL on real environment（OS, browser, IDE）成为开源主线。**simulator → 真实环境 → 部署 RL** 的 sim-to-real pipeline 是下一个 frontier。DeepSeek-V3.2 已公开强调大规模 agent 环境合成和 thinking in tool-use；Anthropic / OpenAI / Google / Meta 等也在产品侧持续推进，但训练细节大多未公开。
...
- 能力持续迭代 Claude 3.5 (new) 2024-10-22 → 3.7 / 4.x / Fable 5 / Opus 4.8
...
- 可能 critic-free（呼应 DeepSeek-R1 / V3.2 与开源 GRPO/RLOO 趋势）
